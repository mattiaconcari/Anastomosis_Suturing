...
ASSETS_DIR = BASE_DIR / "assets"  # folder with assets for simualation

# File di input
STL_FILE = str(ASSETS_DIR / "ago_66mm.stl") 
VESSEL_OBJ = str(ASSETS_DIR / "Arteria_piena.obj")
VESSEL_VTK = str(ASSETS_DIR / "Arteria_piena.vtk")
# Plane loading
p.loadURDF("plane.urdf", [0, 0, 0])

# Function to load an object from stl file
...
# Vessel parameters
vessel_outer_radius = 0.5  # External radius (50 mm)
vessel_length = 1.0  # Vessel length


# Needle with 66mm diameter
radius = 0.33  # 33 mm
angle = 0  # Initial angle
needle_center = [0, 0, 2*vessel_outer_radius]  # To be tuned from optimization
# Needle parameters
needle_position = [0, 0, 2*vessel_outer_radius]  # Initial position
needle_orientation = p.getQuaternionFromEuler([np.pi, 0, 0])  # Initial orientation
...
vessel1 = p.loadSoftBody(
    ...
    basePosition = [-vessel_outer_radius, -0.25, 0],  # Vessel center aligned with Z axis
...
semicircle_center = [0, 0, 2*vessel_outer_radius]
num_suture=6
current_suture=1
...
while p.isConnected():
    ...
    angle += rotation_speed * (1 / 240)  # Angular increment
    if current_suture==1:
